...
def goldapi_get_price(date_iso: str | None = None):
    ...
    base_Gold = "https://www.goldapi.io/api/XAU/USD"
    url = f"{base_Gold}/{date_iso}" if date_iso else base_Gold
    headers = {
...
# Show these sidebar inputs ONLY on Overview
if page == "Overview":
    ...
    # No sidebar filter controls on other pages; use full dataset
    sel_type = []
    search_text = ""
    FILTERED = FUNDS.copy()
...
if page == "Overview":
...
elif page == "Top Funds by Risk":
    st.title("Top Funds by Risk")
    ...
    - **TEFAS (Fund Analysis)** — Official August 2025 dataset from TEFAS.gov.tr  
    - **Fonbul.com** — Used for risk ratings and performance stats  
    - **TCMB & Inflation Verileri** — TUFE (CPI) inflation rates  
    - **Gold Prices** — Historical data via [GoldAPI.io](https://www.goldapi.io)  
    - **USD/TRY Exchange Rates** — [ExchangeRatesAPI.io](https://exchangeratesapi.io), with [ExchangeRate.host](https://exchangerate.host) as backup

    ### How the Data Was Processed
    - Merged multiple CSV files into one clean dataset  
    - Removed duplicates and unnecessary columns  
    - Converted all dates into a proper `datetime` format  
    - Standardized column names for easier handling  
    - Calculated inflation-adjusted returns where possible (`real_1y_return`)

    ### Things to Keep in Mind
    - The dataset is **static** for now — only gold & USD/TRY prices update live  
    - Inflation adjustments are based on **year-over-year TUFE CPI**  
    - Some risk metrics (e.g., Sharpe ratio, max loss) depend on provider formulas

    ### How to Use This App
    - Use the sidebar filters to narrow results by **fund type**, **code**, **company**, etc.  
    - Toggle between **Nominal** and **Inflation-adjusted** returns  
    - Compare multiple funds visually on the charts  
    - Check macro indicators on the **Live Markets** and **TUFE** pages
